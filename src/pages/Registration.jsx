...
export default function Registration() {
  ...
  useSEO(
    event ? event.title : "Registration",
    event ? event.seoDesc : "",
  );

  const [form, setForm] = useState({ name: "", session: "", attendees: 1 });
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch("/api/register", {
        ...
        body: JSON.stringify({ ...form, event: event.eventId }),
      });
      const data = await res.json();
      if (res.ok && data.success) {
        setStatus("success");
      } else {
        const errorKey = data.error === "duplicate_name"
      ...
      <div className="card">
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {sessions.map((s) => {
                ...
                return (
                  <label key={s.value} className="flex items-center gap-3 cursor-pointer">
                    ...
                    {count !== null && (
                      <span className={`ml-auto text-xs px-2 py-0.5 rounded-full whitespace-nowrap font-medium ${badgeClass}`}>
                        {warnLabel ? `⚠ ${warnLabel} · ` : ""}{count} {t("registration.registered")}
                      </span>
                    )}
                  </label>
                );
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">{t("registration.attendeesHint")}</p>
          </div>
